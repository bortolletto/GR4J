...
def simulacao(dt,area, PME, ETP, Qmon, x1, x2, x3, x4, x5, Estados=None):
    ...
    power = 4
    split = 0.9
    # D     = 1.25 # p/ modelos horarios, D = 1.25 (ver Ficchi, 2017, p. 51)
    D = 2.25
    if dt ==24:
        D= 2.5
    else:
...
def erro(X):
       x1, x2, x3, x4, x5= X
       targets = df['vazao']
       predictions = simulacao(dt,A,df["chuva"],df["etp"],0,x1,x2,x3,x4,x5)
       kge,r,alpha,beta = he.evaluator(he.kge,predictions,targets)
       # nash_value = he.evaluator(he.nse, predictions,targets.interpolate("linear"))[0]
...
if __name__ =='__main__':
    ...
    dx = pd.read_csv("./USGS_02430680_combined.csv",index_col = 0,parse_dates = True)  
    # chuva = pd.read_csv("estacoes/CAPIVARI/chuva_media.csv",index_col = 0,parse_dates = True)
    # chuva = pd.read_csv("estacoes/CAPIVARI/chuva_nova.csv",index_col = 0,parse_dates = True)
    # chuva = chuva.resample("H").sum()
    # dx["chuva"] = chuva["medias"]["2000-03-21 10:00:00":"2023-10-23 00:00:00"].values
    # #%%
    import datetime
    # Xmin = [20,-5,20,1,0]
    # Xmax = [1200,3,500,1,1]
    # Xmin = [100,-5,20,1,0]
    # Xmax = [1200,3,300,3,1]
    Xmin = [1,0.1,0.1,0.1,0]
    Xmax = [2000,100,1000,100,1]
    A = 377
    # A = 466.332 # CAPIVARI: 466.332
    # A = 301.6068 # SANTA_FIDELIA MAUA = 301.6068  
    # A = 8923 # TIBAGI AREIA = 8923
        
    # dx = dx.resample('6h').agg({"chuva":np.sum,
    #                                       "vazao":np.mean,
    #                                       "etp":np.sum
    #                                       })
    dt = 24
    ...
    # start_date = datetime.datetime(2015, 3, 25, 0, 0)
    # end_date =  datetime.datetime(2019, 1, 1, 0, 0)
    # mask = (dx.index >= start_date) & (dx.index <= end_date)
    # df_calibracao = dx.loc[mask]
    # df= df_calibracao.copy()
    xbest_f,fbest_f = dds(Xmin,Xmax,erro,r= 0.2,m = 5)
    
    
    # xbeast = [1.00560904e+02, 4.12553468e-01, 1.94933427e+02, 2.99746094e+00,6.14013312e-03]
    #%%
    
    # Q_fore = simulacao(dt,A,df_calibracao["chuva"],df_calibracao["etp"],0,xbest_f[0],xbest_f[1],xbest_f[2],xbest_f[3],xbest_f[4])
   
    Q_fore = simulacao(dt,A,df_calibracao["chuva"],df_calibracao["etp"],0,xbest_f[0],xbest_f[1],xbest_f[2],xbest_f[3],xbest_f[4])
   

    targets = df_calibracao['vazao']
    predictions = Q_fore

  
    # import hydroeval as he
    # kge, r, alpha, beta = he.evaluator(he.kge, predictions, targets)
    # nash_value =  he.evaluator(he.nse, targets.interpolate("linear"), predictions)
    # import plotly.graph_objs as go
    # import plotly.io as pio
    # pio.renderers.default='browser'
    # fig = go.Figure()

    # fig.add_trace(go.Scatter(x = df_calibracao.index, y = df_calibracao.vazao  ,name = "obs",marker_color = "black"))
    # fig.add_trace(go.Scatter(x = df_calibracao.index,y =  Q_fore ,name = "sim", marker_color = "red"))
    # # fig.write_html("/discolocal/felipe/COPEL/estacoes/COPEL/tibagi_areia.html")
    # fig.show()
    #%%
    ...
    # fig = go.Figure()
    # fig.add_trace(go.Scatter(x = df_calibracao.index, y = df_calibracao.vazao  ,name = "obs",marker_color = "black"))
    # lista = [0+x*0.1 for x in range(10)]
    # # lista = [x for x in range(1,100,10)]
    # for numero in lista:
        
    #     Q_fore = simulacao(dt,A,df_calibracao["chuva"],df_calibracao["etp"],0,1.00560904e+02, 0.4, 200, 3,numero)
    #     nash_value =  he.evaluator(he.nse, targets.interpolate("linear"), Q_fore)
    #     fig.add_trace(go.Scatter(x = df_calibracao.index,y =  Q_fore ,name = f'{numero}_{round(nash_value[0])}',))

    # fig.update_layout(title = "x3")
    # fig.show()
    # A = 466.332 # CAPIVARI: 466.332
    # dt = 6
    # xbest_f = [3.67499220e+03, 4.83592922e-01, 9.65316872e+01, 4.99858780e+00,6.20100356e-01]
    # Q_fore = simulacao(dt,A,df_calibracao["chuva"],df_calibracao["etp"],0,xbest_f[0],xbest_f[1],xbest_f[2],xbest_f[3],xbest_f[4])
    # targets = df_calibracao['vazao']
    # predictions = Q_fore
    ...
    # import hydroeval as he
    # kge, r, alpha, beta = he.evaluator(he.kge, predictions, targets)
    # print(kge,nash_value)
    # fig, ax = plt.subplots()
    # line1, = ax.plot(df.index[:21577], df.vazao[:21577], label='Obs. Flows ')
    # line2, = ax.plot(df.index[:21577], Q_fore ,  label=f'sim: nash>{round(nash_value,2)}')
    ...
    # fig.write_html("/discolocal/felipe/COPEL/6h/calibracao.html")
    # fig.show()
    # #%%
    # start_date = datetime.datetime(2015, 1, 1, 0, 0)
    # end_date =  datetime.datetime(2023, 10, 23, 0, 0)
    # mask_valida = (df.index >= start_date) & (df.index <= end_date)
    # df_validacao = df.loc[mask_valida]
    # Q_fore = simulacao(dt,A,df_validacao["chuva"],df_validacao["etp"],0,xbest_f[0],xbest_f[1],xbest_f[2],xbest_f[3],xbest_f[4])
    # targets = df_validacao["vazao"]
    # predictions = Q_fore
    # nash_value = (np.sum((targets-predictions)**2)/np.sum((targets-np.mean(targets))**2))
    # import hydroeval as he
    # kge, r, alpha, beta = he.evaluator(he.kge, predictions, targets)
    # print(kge,nash_value)
    # fig = go.Figure()
    # fig.add_trace(go.Scatter(x = df_validacao.index, y = df_validacao.vazao  ,name = "obs",marker_color = "black"))
    # fig.add_trace(go.Scatter(x = df_validacao.index,y =  Q_fore ,name = "sim", marker_color = "red"))
    # fig.write_html("/discolocal/felipe/COPEL/6h/validacao.html")
    # fig.show()
#%%
# dt = 1
# Q_fore = simulacao(dt,A,df["chuva"],df["etp"],0,xbest_f[0],xbest_f[1],xbest_f[2],xbest_f[3],xbest_f[4])
